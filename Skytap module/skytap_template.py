...
def main():
    module = AnsibleModule(
        argument_spec = dict(
            ...
            action = dict(default='create', choices=['create', 'modify', 'delete', 'list', 'copy', 'read']),
    ...
    if module.params.get('action') == 'copy':
        if not module.params.get('template_id'):
            module.fail_json(msg="template_id is required param when action=copy")
    ...
    if module.params.get('action') == 'read':
        if not module.params.get('template_id'):
            module.fail_json(msg="template_id is required param when action=read")

        status, result = restCall(auth, 'GET', '/v2/templates/'+str(module.params.get('template_id')))
